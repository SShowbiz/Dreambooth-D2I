...
class ProcessDepthMap:
    ...
    def vis_parsing_maps(self, im, dm, eg, parsing_anno, stride, save_im=False, save_path='vis_results/parsing_map_on_im.jpg'):        
        part_colors = [0, 0, EYEBROW_VAL, EYEBROW_VAL, -EYE_VAL, -EYE_VAL, -EYE_VAL, 0, 0, 0, NOSE_VAL, 0, LIP_VAL, LIP_VAL, 0, 0, 0, -HAIR_VAL]
        ...
        eg = eg.convert("L")
        eg = np.array(eg)
        eg[eg < EG_TRHESHOLD] = 0
        max_eg = np.max(eg)
        min_eg = np.min(eg)
        norm_eg = (eg - min_eg)/(max_eg - min_eg) * EG_VAL
        ...
        np_vis = np_dm + vis_parsing_anno_color - norm_eg
